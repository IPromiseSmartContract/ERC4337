...
import { IEntryPoint, UserOpsPerAggregator } from "../interfaces/IEntryPoint.sol";
import { IAccount } from "../interfaces/IAccount.sol";
import { IPaymaster } from "../interfaces/IPaymaster.sol";
import { IAggregator } from "../interfaces/IAggregator.sol";
import { StakeManager } from "./StakeManager.sol";
import { NonceManager } from "./NonceManager.sol";
import { SenderFactory } from "./SenderFactory.sol";

import { UserOperation, ValidationData, StakeInfo, DepositInfo, ReturnInfo } from "../proto/structs.sol";
import { FailedOp, SignatureValidationFailed, SenderAddressResult, ExecutionResult, ValidationResultWithAggregation, ValidationResult } from "../proto/errors.sol";

contract EntryPoint is IEntryPoint, StakeManager, NonceManager, ReentrancyGuard {
    using UserOperationLib for UserOperation;

    SenderFactory private immutable _senderFactory;

    // internal value used during simulation: need to query aggregator.
    address private constant SIMULATE_FIND_AGGREGATOR = address(1);
    // marker for inner call revert on out of gas
    bytes32 private constant INNER_OUT_OF_GAS = hex"deaddead";
    // Get first `REVERT_REASON_MAX_LEN` bytes of revert reason
    uint256 private constant REVERT_REASON_MAX_LEN = 2048;
    // for simulation purposes, validateUserOp (and validatePaymasterUserOp) must return this value
    // in case of signature failure, instead of revert.
    uint256 public constant SIG_VALIDATION_FAILED = 1;

    constructor(SenderFactory _sf) {
        _senderFactory = _sf;
    }
    ...
    function getUserOpHash(UserOperation calldata userOp) public view returns (bytes32) {
        return keccak256(abi.encode(userOp.encodePacked(), address(this), block.chainid));
    }
    ...
    function _createSenderIfNeeded(
        ...
        if (initCode.length != 0) {
            ...
            if (sender.code.length != 0)
                revert FailedOp(opIndex, "[Factory] sender already constructed");
            address sender1 = _senderFactory.createSender{
                gas: opInfo.mUserOp.verificationGasLimit
            }(initCode);
            if (sender1 == address(0))
                revert FailedOp(opIndex, "[Factory] initCode failed or OOG");
            if (sender1 != sender)
                revert FailedOp(opIndex, "[Factory] initCode must return sender");
            if (sender1.code.length == 0)
                revert FailedOp(opIndex, "[Factory] initCode must create sender");
    ...
    function getSenderAddress(bytes calldata initCode) public {
        address sender = _senderFactory.createSender(initCode);
    ...
    function _validateSenderAndPaymaster(
        ...
        if (initCode.length == 0 && sender.code.length == 0) {
            ...
            revert("[Account] account not deployed");
        }
    ...
    function _validateAccountPrepayment(
        ...
        unchecked {
            ...
            returns (uint256 _validationData) {
                ...
                revert FailedOp(opIndex, string.concat("[Account] reverted: ", revertReason));
            } catch {
                revert FailedOp(opIndex, "[Account] reverted (or OOG)");
            }
            if (paymaster == address(0)) {
                DepositInfo storage senderInfo = deposits[sender];
                uint256 deposit = senderInfo.deposit;
                if (requiredPrefund > deposit) {
                    revert FailedOp(opIndex, "[Account] didn't pay prefund");
                }
    ...
    function _validatePrepayment(
        ...
        if (!_validateAndUpdateNonce(mUserOp.sender, mUserOp.nonce)) {
            revert FailedOp(opIndex, "[Account] invalid account nonce");
        }
